...
def _build_param_xml(name: str, value: str) -> str:
    return (
        ...
        "<pub:item>"
        f"<pub:name>{name}</pub:name>"
        f"<pub:values><pub:item>{value}</pub:item></pub:values>"
        "</pub:item>"
        "</pub:parameterNameValues>"
    )
...
def fetch_report_from_oracle(
    ...
    params = ""
    if customer_name:
        params += _build_param_xml("P_CUSTOMER_NAME", customer_name)
    if from_date:
        params += _build_param_xml("P_FROM_DATE", from_date)
    if to_date:
        params += _build_param_xml("P_TO_DATE", to_date)

    envelope = SOAP_TEMPLATE.format(report_path=report_path, parameters=params)
